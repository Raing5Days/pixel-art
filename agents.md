...
pixel-art/
├── AGENTS.md               # 本文件
├── pixel_engine.js          # 核心引擎（对称处理、PNG 编码、模板加载与渲染）
├── base_shapes/             # 预置武器轮廓模板（PNG）
│   ├── broadsword.png
│   ├── short_sword.png
│   ├── staff.png
│   └── throwing_knife.png
├── jungle_series/           # 示例作品集（丛林主题武器）
│   ├── generate_broadsword.js
│   ├── generate_broadswords.js
│   ├── generate_jungle_staff.js
│   └── ...
└── prompts/                 # （可选）描述文本 → 配置转换
```
